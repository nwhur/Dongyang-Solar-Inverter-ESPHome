...
graph LR
    ...
    MOD_VCC -->|점퍼선| ESP_3V3
    MOD_GND -->|점퍼선| ESP_GND
    MOD_TX  -->|점퍼선 (교차)| ESP_RX
    MOD_RX  -->|점퍼선 (교차)| ESP_TX
```
...
### 3단계: 인버터 하부 커버 분리 및 통신 단자 찾기
1. 인버터의 DC 스위치와 AC 차단기가 모두 차단되었는지 확인합니다.
2. 인버터 하단부의 고정 나사(보통 4개)를 십자드라이버로 풀고 배선실 커버를 조심스럽게 엽니다.
3. 커버 안쪽 PCB 기판 또는 단자대에서 **`RS-485`**, **`485+` / `485-`**, 또는 **`A` / `B`**로 표기된 단자를 찾습니다.
   * 일부 모델은 나사식 터미널 블록 형태이며, 일부 모델은 RJ45(랜선) 포트 형태로 되어 있습니다.
   * 나사식 단자대의 경우 `A` 단자에 통신선의 1번 선, `B` 단자에 2번 선을 물리고 단단히 조여줍니다.

### 4단계: 인버터 통신 ID 설정 (중요!)
동양 인버터는 질의 시 본인의 ID와 일치할 때만 응답을 보냅니다.
* **1대만 사용할 경우 (기본):**
  * 인버터 ID가 `2` (`0x02`)로 설정되어 있어야 합니다.
  * 인버터 전면 LCD의 `Setting` 메뉴 또는 기판 내 딥스위치(DIP Switch)를 확인하여 ID가 `2`인지 확인합니다. (공장 출하 기본값이 보통 1 또는 2입니다.)
  * 💡 **꿀팁:** 만약 인버터 ID가 이미 `1`로 고정되어 있거나 딥스위치 변경이 번거로운 경우, 인버터를 건드릴 필요 없이 `solar_inverter_single.yaml`의 `my_inv_global->set_inv1_id(0x01);` 로 수정하시면 소프트웨어에서 즉시 ID 1번으로 자동 통신합니다!
* **2대 병렬(Dual)로 사용할 경우:**
  * **1번 인버터:** ID를 `2` (`0x02`)로 설정합니다.
  * **2번 인버터:** ID를 `97` (`0x61`)로 설정합니다.
  * **배선:** 1번 인버터의 A와 2번 인버터의 A를 서로 연결하고, B와 B를 서로 연결(데이지 체인)한 뒤, 최종적으로 RS-485 모듈의 A, B에 물려줍니다.
